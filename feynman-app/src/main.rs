            ...
            match e {
                ...
                openai_realtime::types::events::ServerEvent::ConversationItemInputAudioTranscriptionCompleted(data ) => {
                    ...
                    if topic_change {
                        ...
                        {
                            Ok(a) => {
                                println!("Questions/Analysis generated by analyze_topic: {}", a);
                                a
                            }
                            Err(e) => {
        ...
        while let Some(i) = input_rx.recv().await {
            match i {
                Input::Initialize() => {
                    ...
                    let instructions = r#"You are a student who is smart and very curious, but 'dumb' in the sense that you know nothing about the topic except what the teacher just explained. When you receive a question, speak it out loud as if you are asking for clarification or expressing confusion. Do not invent information or answer questions—just ask them. Always act as a persistent, curious student."#;
